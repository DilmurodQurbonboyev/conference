            ...
            <div class="col-md-12 my-2 d-flex justify-content-between">
                @can('useful-category.create')
                    <a class="btn btn-primary create-btn"
                       href="{{ route('useful-category.create') }}">{{ tr('Create Useful Category') }}</a>
                @endcan
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-hover table-wrap">
                ...
                <tr>
                    ...
                    <th>
                        <select class="form-control" wire:model.debounce.300ms="filter_parent_id">
                            ...
                            @foreach ($usefulParent as $useful_parent)
                                <option value="{{ $useful_parent->id }}">{{ $useful_parent->title }}</option>
                            @endforeach
                        </select>
                    </th>
                    <th>
                ...
                @foreach ($usefuls as $key => $useful)
                    <tr>
                        <td>{{ ++$key }}</td>
                        <td scope="row">{{ $useful->id }}</td>
                        <td>
                            @if ($useful->parent !== null)
                                {{ $useful->parent->title }}
                            @else
                                {{ tr('Main Category') }}
                            @endif
                        ...
                        <td class="d-flex">
                            @can('useful-category.show')
                                <a class="btn btn-primary m-1"
                                   href="{{ route('useful-category.show', $useful->id) }}" title="View"
                                   aria-label="View"><span class="fas fa-eye"></span>
                                </a>
                            @endcan
                            @can('useful-category.edit')
                                <a class="btn btn-primary m-1"
                                   href="{{ route('useful-category.edit', $useful->id) }}" title="Янгилаш"
                                   aria-label="Янгилаш"><span class="fas fa-pencil-alt"></span>
                                </a>
                            @endcan
                            @can('useful-category.destroy')
                                <form action="{{ route('useful-category.destroy', $useful->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" wire:click="deleteId({{ $useful->id }})"
