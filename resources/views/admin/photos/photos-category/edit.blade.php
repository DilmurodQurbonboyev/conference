...
@section('header')
    <div class="col-sm-6">
        <h1>{{tr('Update Photos Category')}}</h1>
    </div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            {{ Breadcrumbs::render('photos-category/edit', $photosCategory->id) }}
        </ol>
    </div>
@endsection
@section('content')
            ...
            <form action="{{ route('photos-category.update', $photosCategory->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 select2" name="parent_id" id="parent_id">
                        ...
                        @foreach ($photosCategories as $photo)
                            <option
                                value="{{ $photo->id }}" {{ $photosCategory->parent_id == $photo->id ? 'selected' : '' }}>{{ $photo->title }}</option>
                        @endforeach
                    </select>
                </div>
                        ...
                        <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
                            <div class="form-group">
                                ...
                                <input type="text" name="title_{{ $lang }}"
                                       ...
                                       id="title_{{ $lang }}"
                                       value="{{ $photosCategory->translate($lang)->title ?? '' }}">
                                @error('title_'.$lang)
                ...
                <div class="form-group">
                    ...
                    <div class="input-group my-colorpicker2">
                        ...
                        <input type="text" name="color" class="form-control" value="{{ $photosCategory->color }}">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <img class="p-2" src="{{ $photosCategory->image }}" width="30%" alt="">
                </div>
                <div class="form-group">
                    <label for="status">{{ tr('Status') }}</label>
                    <select class="custom-select rounded-0 select2" name="status" id="status">
                        <option
                            value="2" {{ $photosCategory->status == "2" ? 'selected' : '' }}>{{tr('Active')}}</option>
                        <option
                            value="1" {{ $photosCategory->status == "1" ? 'selected' : '' }}>{{tr('Inactive')}}</option>
                    </select>
                </div>
                <div class="form-group">
