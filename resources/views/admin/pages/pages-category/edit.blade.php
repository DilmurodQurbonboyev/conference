...
@section('title')
    {{tr('Update Pages Category')}}
@endsection
@section('header')
    <div class="col-sm-6">
        <h1>{{tr('Update Pages Category')}}</h1>
    </div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            {{ Breadcrumbs::render('pages-category/edit', $pagesCategory->id) }}
        </ol>
    </div>
@endsection
@section('content')
            ...
            <form action="{{ route('pages-category.update', $pagesCategory->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 select2" name="parent_id" id="parent_id">
                        ...
                        @foreach ($pagesCategories as $pagesCategory)
                            <option
                                value="{{ $pagesCategory->id }}" {{ $pagesCategory->parent_id == $pagesCategory->id ? 'selected' : '' }}>{{ $pagesCategory->title }}</option>
                        @endforeach
                    </select>
                </div>
                        ...
                        <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
                            <div class="form-group">
                                ...
                                <input type="text" name="title_{{ $lang }}"
                                       ...
                                       id="title_{{ $lang }}"
                                       value="{{ $pagesCategory->translate($lang)->title ?? '' }}">
                                @error('title_'.$lang)
                ...
                <div class="form-group">
                    ...
                    <div class="input-group my-colorpicker2">
                        ...
                        <input type="text" name="color" class="form-control" value="{{ $pagesCategory->color }}">
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 select2" name="status" id="status">
                        <option
                            value="2" {{ $pagesCategory->status == "2" ? 'selected' : '' }}>{{tr('Active')}}</option>
                        <option
                            value="1" {{ $pagesCategory->status == "1" ? 'selected' : '' }}>{{tr('Inactive')}}</option>
                    </select>
                </div>
                <div class="form-group">
