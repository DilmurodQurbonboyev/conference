...
@section('header')
<div class="col-sm-6">
    <h1>{{tr('Update Management Category')}}</h1>
</div>
<div class="col-sm-6">
    <ol class="breadcrumb float-sm-right">
        {{ Breadcrumbs::render('managements-category/edit', $managementCategory->id) }}
    </ol>
</div>
@endsection
@section('content')
                ...
                <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
                    <div class="form-group">
                        ...
                        <input type="text" name="title_{{ $lang }}"
                            ...
                            value="{{ $managementCategory->translate($lang)->title ?? '' }}">
                        @error('title_'.$lang)
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    <input id="thumbnail2" class="form-control" type="text" name="image"
                        value="{{ $managementCategory->image }}">
                ...
                <img class="p-2" src="{{ $managementCategory->image }}" width="30%" alt="">
            </div>
            <div class="form-group">
                <label for="status">{{ tr('Status') }}</label>
                <select class="custom-select rounded-0 select2" name="status" id="status">
                    <option value="2" {{ $managementCategory->status == "2" ? 'selected' : '' }}>{{tr('Active')}}
                    </option>
                    <option value="1" {{ $managementCategory->status == "1" ? 'selected' : '' }}>{{tr('Inactive')}}
                    </option>
                </select>
            </div>
            <div class="form-group">
