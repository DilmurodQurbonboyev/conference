...
class PostersCategory extends Component
{
    public $perPage = 10;
    public $filter_id,
        $filter_title,
    ...
    public function render()
    {
        ...
        $query = ListCategory::query()->where('list_type_id', 2);

        $postersParent = ListCategory::where('list_type_id', 2)->where('parent_id', 0)->get();
        ...
        $posters = $query->paginate($this->perPage);

        return view('livewire.posters-category', compact('users', 'postersParent', 'posters'));
    }

    public function export($format)
    {
        return Excel::download(new ListCategoryExport, "posters-category.$format");
    }
